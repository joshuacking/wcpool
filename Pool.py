...
class Pool:
    ...
    def __init__(self, pool_id):
        ...
        self.draft['status'] = 'NOT_STARTED'
        self.draft['picks'] = []
        self.last_updated = ""

    def create(self, pool, season_id, entries, admin, domain):
        """Creates a new pool"""
        ...
        try:
            ...
            raise Exception("Entries should be an integer indicating the number of entries in the pool.")
            #return '*Error*\nentries should be an integer indicating the number of entries in the pool.'

        # make sure season is valid
        # this will throw an exception if invalid, which we'll let pass through
        season = Season(season_id)
        season.retrieve()

        self.dao.new_pool(pool, season_id, entries, admin, domain)
    ...
    def get_entry_by_team(self, team_id):
        for entry in self.entries:
            for team in self.entries[entry]['teams']:
                if team_id == team:
                    return self.entries[entry]['user_name']
    ...
    def set_expected_entries(self, expected_entries=0):
    ...
    def retrieve(self):
        ...
        #self.set_name(pool_id)
        self.dao.get_pool(self)
        self.retrieved = True
